...
sidimedDBclient = MongoClient(
    'mongodb://sidimedsDBuser:sidimedsDBpassword@ec2-52-209-2-160.eu-west-1.compute.amazonaws.com:27017/sidimedsDB')
...
def create_ad(request):
    ...
    ad_model = {
        ...
        "publisher": {
            ...
            # make a password gives different results for the same password, see that later
...
def updateAd(request):
    ...
    ad = ads_advertisement.update_one(
        {'_id': ObjectId(_id)}, {"$set":  obj}, upsert=True)
    print(ad)
...
def upload_my_file(bucket, folder, region, file):
    ...
    key = folder+'/'+str(round(time.time() * 1000)) + \
        split_tup[len(split_tup)-1]
    print('key = '+key)
    try:

        response = s3_client.upload_fileobj(file, bucket, key, ExtraArgs={
...
def handle_uploaded_file(f):
    ...
    name = 'uploads/'+str(round(time.time() * 1000)) + \
        split_tup[len(split_tup)-1]
...
def getUniqueVisitorsCount(request):
    ...
    today = date.today()
    tonightAtmidnight = datetime.datetime.combine(
        today, datetime.datetime.min.time())

    cursor = statistics.distinct("id", {"page": "map", "visitDate": {
                                 "$lt": datetime.datetime.now(), "$gte":  tonightAtmidnight}})
...
def getUniqueVisitorsLast24Hours(request):
    ...
    cursor = statistics.distinct(
        "id", {"page": "map", "visitDate": {"$gte":  last24Hours}})
...
def getUniqueVisitorsLast30Days(request):
    ...
    cursor = statistics.distinct(
        "id", {"page": "map", "visitDate": {"$gte":  last30days}})
    counter = len(list(cursor))
...
def getVisistsCountLast24Hours(request):
    ...
    last24Hours = datetime.datetime.now() - datetime.timedelta(days=1)
    cursor = statistics.find(
        {"page": "map", "visitDate": {"$gte":  last24Hours}})
    counter = len(list(cursor))
    return JsonResponse({"uniqueVisitors": counter}, safe=False)
...
def getVisitsCountLast365Hours(request):
    ...
    cursor = statistics.find(
        {"page": "map", "visitDate": {"$gte":  last365Hours}})
    counter = len(list(cursor))
...
def getUniqueVisitorsOfLast365Days(request):
    ...
    cursor = statistics.distinct(
        "id", {"page": "map",  "visitDate": {"$gt":  lastyear}})
...
def getUniqueVisitorsThatVisitedForTheFirstTimeEverInTheLast24Hours(request):
    ...
    cursor2 = statistics.distinct("id", {"page": "map",  "visitDate": {
                                  "$lt":  last24Hours}})  # all time minus 24 hours
...
def getUniqueVisitorsThatVisitedForTheFirstTimeEverInTheLast30Days(request):
    ...
    cursor2 = statistics.distinct("id", {"page": "map",  "visitDate": {
                                  "$lt":  last30Days}})  # all time minus 30 hours
...
def getRegionsOfCountry(request):
    ...
    input = JSONParser().parse(request)
    cursor = statistics.distinct(
        "region", {"page": "map", "country": str(input['country'])})
...
def getCitiesOfRegion(request):
    ...
    input = JSONParser().parse(request)
    cursor = statistics.distinct("city", {"page": "map", "country": str(
        input['country']), "region": str(input['region'])})
...
def getCountriesForLast24Hours(request):
    ...
    last24Hours = datetime.datetime.now() - datetime.timedelta(days=1)
    cursor = statistics.distinct(
        "country", {"page": "map",  "visitDate": {"$gte":  last24Hours}})
    json_ads = []
...
def getAllCountriesThatvisitedForFirstTimeInLastMonth(request):
    ...
    cursor1 = statistics.distinct(
        "country", {"page": "map", "visitDate": {"$lte": dateOfStart}})
    ...
    difference = set(countries_list_Alltime) - \
        set(countries_list_except_last_month)
...
@api_view(['GET'])
# get unique visitors from and from outside Mauritania
def getUniqueVisitorsFromAndFromOutsideMauritania(request):

    cursorMauritania = statistics.distinct(
        "id", {"page": "map", "country": "Mauritania"})
    ...
    for i in range(len(x)):
        plt.text(i, y[i],  str(y[i]) + " (" + str(float("{0:.1f}".format(
            (y[i]/counterTotal) * 100))) + "%)", ha="center", va="bottom")
...
def getUniqueVisitorsPerMonth(request):
    ...
    for doc in cursor:
        ...
        if item['_id']['month'] > 9:
            ...
            my_month = str(item['_id']['year']) + '-' + \
                '0'+str(item['_id']['month'])
    ...
    list_months_counts_sorted = dict(
        sorted(list_months_counts.items(), key=lambda item: item[0],  reverse=False))
    base64_list_months_counts_sorted = generate_base64_image(
        list_months_counts_sorted, "Visiteurs Uniques par Mois", rotate)
...
def getUniqueVisitorsPerOs(request):
    ...
    # list_months_counts_sorted = sorted(list_months_counts, key=lambda k: k['month'], reverse=False)
    list_os_counts_sorted = dict(
        sorted(list_os_counts.items(), key=lambda item: item[1],  reverse=True))
    base64_list_os_counts_sorted = generate_base64_image(
        list_os_counts_sorted, "Visiteurs Uniques par Systèmes d'Exploitation durant les derniers 365 jours", rotate)
...
def getUniqueVisitorsPerDevice(request):
    ...
    # list_months_counts_sorted = sorted(list_months_counts, key=lambda k: k['month'], reverse=False)
    list_device_counts_sorted = dict(
        sorted(list_device_counts.items(), key=lambda item: item[1],  reverse=True))
    base64_list_device_counts_sorted = generate_base64_image(
        list_device_counts_sorted, "Visiteurs Uniques Par Appareil durant les derniers 365 jours", rotate)
...
def getUniqueVisitorsPerCity(request):
    ...
    list_cities_counts_sorted = dict(
        sorted(list_cities_counts.items(), key=lambda item: item[1],  reverse=True))
...
def getUniqueVisitorsPerRegion(request):
    ...
    list_regions_counts_sorted = dict(
        sorted(list_regions_counts.items(), key=lambda item: item[1],  reverse=True))
...
def getUniqueVisitorsPerCountry(request):
    ...
    base64_list_mauritania_abroad_counts = generate_base64_image(
        list_mauritania_abroad_counts, "Visiteurs Uniques Provenant de la Mauritanie et de l'Etranger ", rotate)
    ...
    sorted_list_foreign_country_counts = sort_countries(
        list_foreign_country_counts)

    sorted_and_trimed_countries = sort_and_trim_countries(
        list_foreign_country_counts)

    base64_list_foreign_country_counts_sorted = generate_base64_image(
        sorted_and_trimed_countries, "Visiteurs Uniques Provenant de l'Etranger ", rotate)
...
def getUniqueVisitorsPerCountryDuringLast24Hours(request):
    ...
    sorted_countries = dict(
        sorted(countries.items(), key=lambda item: item[1],  reverse=True))
...
def getGraphs(request):
    ...
    sorted_mauritania_other_404 = dict(
        sorted(mauritania_other_404.items(), key=lambda item: item[1],  reverse=True))
    base64_string_mauritania_other_404 = generate_base64_image(
        sorted_mauritania_other_404, "Visiteurs Uniques provenant de la Mauritanie et de l'étranger", False)

    sorted_and_trimed_countries = sort_and_trim_countries(countries)
    base64_string_countries = generate_base64_image(
        sorted_and_trimed_countries, "Visiteurs uniques par pays étranger", False)

    sorted_dates = sort_dates_keys(dates)
    base64_string_dates = generate_base64_image(
        sorted_dates, "Visiteurs uniques par mois ayant visité l'application pour la première fois", False)

    sorted_devices = dict(
        sorted(devices.items(), key=lambda item: item[1],  reverse=True))
    base64_string_devices = generate_base64_image(
        sorted_devices, "Visiteurs uniques par appareil", False)

    sorted_opearting_systems = dict(
        sorted(opearting_systems.items(), key=lambda item: item[1],  reverse=True))
    base64_string_opearting_systems = generate_base64_image(
        sorted_opearting_systems, "Visiteurs uniques par systèmes d'exploitation", False)
...
def generate_base64_image(stats, title, rotate):
    ...
    plt.xlabel("Total = "+str(total)+"(100%) , timestamp : " +
               str(my_timestamp_trimmed), weight='bold', fontsize=12)
...
def rotate_image(image, angle):
    ...
    new_width = int(abs(width * math.cos(radians)) +
                    abs(height * math.sin(radians)))
    new_height = int(abs(height * math.cos(radians)) +
...
@api_view(['GET'])
def nouvellesdemandes(request):

    json_ads = [{
        "client": "INV_AD_ID",
        "nud": "84029052023101737C0009",
        "date_demande": "29/05/2023",
        "request_id": 1004532001,
        "pin": "5771111419",
        "statut_demande": None,
        "commentaires": " ",
        "type_document_demande": "ID"
    }, {
        "client": "CHECK",
        "nud": "01429052023100133C0001",
        "date_demande": "29/05/2023",
        "request_id": 1004532003,
        "pin": None,
        "statut_demande": None,
        "commentaires": " ",
        "type_document_demande": None
    }]
...
@api_view(['POST'])
def getValidated(request):

    json_ads = [
        {
            "nud": "84024052023142342C0044",
            "mrz": "P<MRTSIDINA<<MEINNA<<<<<<<<<<<<<<<<<<<<<<<<<B013796300MRT8511087F2805232<<<<<<<<<<<<<<04",
            "document_number": "B01379630"
        },
        {
            "nud": "84102022023114534C0040",
            "mrz": "I<MRTIG080093807552896950<<<<<9601053F3502066MRT<<<<<<<<<<<4MOHAMED<KABER<<KHAWLA<<<<<<<<<",
            "document_number": "IG0800938"
        }
    ]
...
def getPaid(request):
    ...
    json_ads = [{
        ...
        "commentaires": "[ecaabeidy] {07/02/2023}  "
    }]
...
@api_view(['POST'])
def getgeneratedNni(request):

    json_ads = [{
        "nud": "03329052023093056C0015",
        "pin": "1034706470",
        "commentaires": "تم التقييد على اساس  حكم ميلاد وعقد زواج ومحضر تصريح تم توقيعه من طرف الاب ورئيس المركز\n "
    },
        {
        "nud": "96729052023091415C0005",
        "pin": "7331898455",
        "commentaires": "تم التقييد على اساس  حكم ميلاد وعقد زواج ومحضر تصريح تم توقيعه من طرف الاب ورئيس المركز\n "
    }]
